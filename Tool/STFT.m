...
elseif (mod(fftSize,shiftSize) ~= 0)
    fprintf('%d \n', mod(fftSize,shiftSize));
    error('The third argument must be dividable by the fourth argument.\n');
end
if (nargin<4)
    ...
    if (isnumeric(window))
        if (size(window, 1) ~= fftSize)
            error('The length of the forth argument must be the same as that of second argument.\n');
        end
    else
        switch window
            case 'hamming'
...
%for ch = 1:nch
    for j = 1:frames
        sp = (j-1)*shiftSize;
        spectrum = fft( signal(sp+1:sp+fftSize) .* window );
       % S(:,j,ch) = spectrum(1:fftSize/2+1, 1); % freq. x frame x ch
        S(:,j) = spectrum(1:fftSize/2+1); % freq. x frame x ch
    end
%end
end
